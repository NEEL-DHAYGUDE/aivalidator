...
# CONFIGURATION
# ==========================================
GOOGLE_SCRIPT_URL = "[https://script.google.com/macros/s/AKfycbwLrFTMKtPGozjmKez8Za2ubTGdKVJnO_G5OlvOMltHdR2kIGvkRZ9rxYKzPJzhr1J7/exec](https://script.google.com/macros/s/AKfycbwLrFTMKtPGozjmKez8Za2ubTGdKVJnO_G5OlvOMltHdR2kIGvkRZ9rxYKzPJzhr1J7/exec)"
...
<head>
    ...
    <style>
        ...
        /* PDF PRINT CLEANING LOGIC */
        @media print {
            .navbar, .footer, button, .api-note, hr, .navbar-brand, .navbar-links {
        ...
        if api_key and master_file and target_file:
            try:
                ...
                return render_template_string(HTML_BASE, content=result_content)
                 ...
                 error_content = f"<div class='container'><h1>Analysis Failed</h1><p>{str(e)}</p><a href='/dashboard'><button>Return</button></a></div>"
    ...
    dashboard_content = f"""
        <div class="container">
            ...
            <form method="POST" enctype="multipart/form-data" onsubmit="showLoader('EXECUTING COMPLIANCE AUDIT...')">
                ...
                <input type="password" name="api_key" placeholder="Mistral API Key" required>
                <h3 style="margin-top: 30px;">2. Document Ingestion</h3>
                <label>Master Specification (Rules)</label>
                <input type="file" name="master_doc" accept=".pdf" required>
                <label>Vendor Proposal (Target)</label>
                <input type="file" name="target_doc" accept=".pdf" required>
                <button type="submit" style="margin-top: 40px;">EXECUTE AI COMPLIANCE AUDIT</button>
            </form>
            <div style="text-align: center;"><a href='/logout' style='color: #ef4444;'>[ Secure Logout ]</a></div>
        </div>
    """
...
def run_ai_audit(api_key, master_text, target_text):
    ...
    DOCUMENT 1: COMPANY MASTER SPECIFICATIONS: {master_text}
    DOCUMENT 2: VENDOR PROPOSAL: {target_text}
    
    TASK: 
    1. Compare Document 2 against Document 1. 
    2. Give a "Safety & Compliance Score" out of 100.
    3. Explain why. List all critical deviations.
    
    FORMATTING: Use ONLY basic HTML (<h2>, <p>, <b>, <ul>, <li>). Raw HTML only. No backticks.
    """
    client = Mistral(api_key=api_key.strip())
    response = client.chat.complete(
        model="mistral-large-latest", 
        messages=[
            {"role": "system", "content": "Output clean HTML only. No markdown, no backticks."},
            {"role": "user", "content": prompt}
        ],
        temperature=0.1
    )
    # CLEANING LOGIC: Remove ```html and ``` if present
